...
| **起こしたいPC**（電源を入れたい側） | Wake 要求の受信・マジックパケットの送信 | **必要** |
...
- **TCPによる起動確認**: Wake 送信後に「本当に起動できたか」を確認する場合、起こしたいPC上の wol-relay の HTTP ポート（例: 8080）に接続して確認します。wol-relay がインストールされて動作していれば、このポートが開くため確認できます。また、wol-relay は起動時に Windows ファイアウォールのアクセス許可ダイアログを出すので、追加のファイアウォール設定が不要です。
- **マジックパケットの検知・再送**: 起こしたいPCと同じセグメントに wol-relay があれば、別セグメントから届いた Wake 要求をマジックパケットとして再送できます。

同じLAN内（ルーターを挟まない）のPCを起こすだけであれば、起こしたいPCへのインストールは必須ではありません。別セグメントのPCを起こす場合や、起動確認を使いたい場合にインストールしてください。
...
Windows では、インストール後にスタートメニューまたはデスクトップショートカットの `wol-relay Wake on LAN` を開きます。macOS では `/Applications` にコピーした `wol-relay Wake on LAN.app`、Linux ではアプリケーション一覧の `wol-relay` を開きます。
...
軽量モードは Raspberry Pi などの Agent 常駐向けです。GUI からは切り替えず、Linux パッケージに含まれる systemd service、または CLI の `wol-relay agent -light` で使います。誤って設定ファイルに `lightweight: true` が残っていても、`wol-relay gui` は設定を直せるように GUI を開きます。

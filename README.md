...
ネイティブGUIは起動時にウィンドウやコンソールを表示せず、タスクトレイ／メニューバーのインジケーターに常駐します。インジケーターの左クリックまたはメニューの「wol-relay を表示」で設定画面を開けます。ウィンドウの閉じるボタンまたは「インジケーターに最小化」を押すと再びインジケーターだけの状態になり、完全に終了する場合はメニューの「終了」を選びます。

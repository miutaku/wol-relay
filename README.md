...
配布された zip / tar.gz を展開し、OS に合わせてインストールします。

Windows:

`wol-relay-installer.exe` をダブルクリックし、セットアップウィザードに従ってインストールします。

アンインストールする場合は、`wol-relay-uninstaller.exe` をダブルクリックし、ウィザードに従って削除します。設定ファイルを残すか削除するかを選べます。
...
Windows では、インストール後にデスクトップの `wol-relay.exe` をダブルクリックします。macOS / Linux では、アプリケーション一覧の `wol-relay` を開きます。
...
Windows 向けには、ダブルクリックで使える単体インストールウィザードとアンインストールウィザードもビルドします。この `wol-relay-installer.exe` にはアプリ本体の機能も含まれており、インストール時にユーザー領域へ `wol-relay.exe` として配置されます。
...
- Ubuntu + MinGW による Windows 配布バイナリのビルド確認
...
- `wol-relay_<version>_windows_amd64.zip`
- `wol-relay_<version>_linux_amd64.tar.gz`
- `wol-relay_<version>_darwin_amd64.tar.gz`
- `wol-relay_<version>_darwin_arm64.tar.gz`

Windows 向け zip には、アプリ本体、インストーラー、アンインストーラーを含めます。Linux / macOS 向け tar.gz には、アプリ本体、インストールスクリプト、アンインストールスクリプト、README、LICENSE を含めます。
